...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # print('year= '+year, 'rank='+rank, 'name='+name)
    if name in name_data:
        if year in name_data[name]:
            original_rank = int(name_data[name][year])
            if original_rank > int(rank):
    ...
    with open(filename, 'r') as f:
        ...
        for line in f:
            if n == 0:
                ...
                rank = line_list[0].strip()
                name1 = line_list[1].strip()
                name2 = line_list[2].strip()
...
def search_names(name_data, target):
    ...
    for name in name_data:
        if name.find(target1) != -1:
            matching_names.append(name)
        elif name.find(target2) != -1:
            matching_names.append(name)
